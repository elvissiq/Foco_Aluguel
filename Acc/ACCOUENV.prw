...
method processaTemp(cJSON, cId, nPart, cCard) class ACCOUENV
    ...
    cPath := "/rest/v1/ledger/load/"+ cId +"/upload/" + cValToChar(nPart)
    ...
    if self:oRestClient:ORESPONSEH:CSTATUSCODE == "200"
        ...
        clVol := .F.
        MakeDir("\AccountfyError")
        cArqLog := "\AccountfyError" + cId + " - Parte " + cValToChar(nPart) + ".txt"
        MEMOWRITE( cArqLog, cJSON )
...
method confirmaProcesso(cId,cCard) class ACCOUENV
    ...
    cPath := "/rest/v1/ledger/load/"+ cId +"/process"
...
method putId(cCod,cIdPart) class ACCOUENV
	...
	if ZKV->(dbSeek( xFilial("ZKV") + cCod))
        ...
        cPath := "/rest/v1/ledger/load/" + alltrim(cIdPart) + "/cancel"  
