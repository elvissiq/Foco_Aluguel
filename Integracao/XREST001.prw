...
/*/ Rotina XREST001
  ...
  WebService REST para integração com o módulo Financeiro.
...
/*/
// ---------------------------------------------------------
WsRestFul XREST001 Description "API Generica Protheus"

  WsMethod Get         Description "Consulta tabela Protheus" WSSYNTAX "/api/retail/v1/XREST001" PATH "/api/retail/v1/XREST001"
  WsMethod Get Auditor Description "Consulta Protheus Auditoria" WSSYNTAX "/api/retail/v1/XREST001/Auditor" PATH "/api/retail/v1/XREST001/Auditor"
  WsMethod Post        Description "Inclusao no PROTHEUS" WSSYNTAX "/api/retail/v1/XREST001" PATH "/api/retail/v1/XREST001"

End WsRestFul

//--------------------------------------------------
/*/{protheusDoc.marcadores_ocultos} XREST001
...
/*/
//--------------------------------------------------
WsMethod Get WsReceive RECEIVE WsService XREST001
  Local lRet     := .F.
...
/*/
//--------------------------------------------------
WsMethod Get Auditor WsReceive RECEIVE WsService XREST001
  Local lRet     := .F.
  Local oJson	  := THashMap():New()
  Local cMsg     := ""
  Local cJsonRet := ""
...
/*/
//-----------------------------------------------------
WsMethod Post WSReceive RECEIVE WsService XREST001
  Local oJson	    := THashMap():New()
  Local cMensag    := ""
  Local cJsonRet   := ""
  Local cJson      := ""
...
Static Function fnR01Grv(oJson, cJson, cMensag)
 ...
 // --------------------------------------------------
  cFileCtr := cRootPath + cBarras + "log_json" + cBarras + "JSON_" + cFilAnt + "_" + cIDRot + ".ctr"
 ...
 // --------------------------------------------------- 
  ...
  If AttIsMemberOf(oJson,"itens") 
     ...
     cFile    := cRootPath + cBarras + "log_json" + cBarras + cArqJson
